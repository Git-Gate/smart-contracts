...
contract POGMFactory {
 ...
 /**
   ...
   */
    function createPOGMToken(uint256 tokenizedRepoId) external returns (address) {
        if (POGMs[tokenizedRepoId] != address(0)) revert Already_Created(POGMs[tokenizedRepoId]);

        POGMRegistry instanceRegistry = POGMRegistry(POGMRegistryAddress);
        address repoOwner = instanceRegistry.getTokenizedRepoOwner(tokenizedRepoId); 
        string memory tokenizedRepoName = instanceRegistry.getTokenizedRepoName(tokenizedRepoId);
        string memory POGMBaseUri = instanceRegistry.getSoulBoundBaseURI(tokenizedRepoId);

        if (repoOwner == address(0)) revert Not_Existing_Repo();

        address POGMAddress = address(new POGMToken(tokenizedRepoName, tokenizedRepoId, POGMRegistryAddress, POGMBaseUri)); 
 ...
 /**
   ...
   */
    function _setTokenizedRepoPOGMAddress(uint256 tokenizedRepoId, address POGMAddress) internal {
        POGMRegistry instanceRegistry = POGMRegistry(POGMRegistryAddress);
        instanceRegistry.setSoulboundAfterCreation(tokenizedRepoId, POGMAddress);
    }
}